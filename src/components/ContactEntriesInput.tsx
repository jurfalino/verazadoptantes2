...
/**
 * Contact input with two switchable modes:
 *  - 'fields' — editable typed-chip rows + manual add.
 *  - 'paste'  — a single paste box; a paste/categorize APPENDS typed entries
 *               (never replaces), then returns to 'fields' so the result shows.
 * A new adopter (no entries) opens in 'paste'; an existing one in 'fields'.
 * Toggling only swaps the visible surface — `entries` is never touched.
 */
export default function ContactEntriesInput({ entries, onChange }: ContactEntriesInputProps) {
    const { t } = useLanguage();
    const [draft, setDraft] = useState('');
    const [mode, setMode] = useState<'fields' | 'paste'>(() => (entries.length === 0 ? 'paste' : 'fields'));
    ...
    return (
        <div className="space-y-3">
            <div className="flex justify-end">
                <button
                    type="button"
                    onClick={() => setMode(mode === 'fields' ? 'paste' : 'fields')}
                    className="text-sm font-medium text-teal-700 hover:opacity-70 transition-opacity"
                >
                    {mode === 'fields' ? t('adopter.ce_mode_paste') : t('adopter.ce_mode_fields')}
                </button>
            </div>
            ...
            {mode === 'paste' ? (
                <div className="space-y-2">
                    ...
                    {entries.length > 0 && (
                        <p className="text-xs text-stone-500">{t('adopter.ce_paste_keeps')}</p>
                    )}
                </div>
            ) : (
                <>
