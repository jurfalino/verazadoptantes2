...
export default function InstallCTA() {
    ...
    return (
        <div className="mt-10 mx-auto max-w-md">
            <div className="rounded-2xl p-5 text-center" style={{ background: 'var(--surface-card)', border: '1px solid var(--border-default)' }}>
                ...
                <div className="space-y-2 mb-4 text-left">
                    {benefits.map((b, i) => (
                        <div key={i} className="flex items-center gap-2.5 px-3 py-1.5 rounded-lg" style={{ background: 'var(--surface-elevated)' }}>
                            <span className="text-base">{b.icon}</span>
                            <span className="text-xs" style={{ color: 'var(--text-primary)' }}>{b.text}</span>
                        </div>
                    ))}
                </div>
                ...
                {/* Action */}
                {isIOS ? (
                    <p className="text-xs rounded-lg px-3 py-2" style={{ color: 'var(--text-muted)', background: 'var(--surface-elevated)' }}>
                        {t('pwa.ctaIOSHint') || 'Tap'} <span className="inline-flex items-center align-middle mx-0.5"><svg className="w-4 h-4" fill="none" stroke="var(--brand)" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg></span> {t('pwa.ctaIOSAction') || 'then "Add to Home Screen"'}
                    </p>
                ) : (
                    <button
                        onClick={handleInstall}
                        className="w-full py-2.5 font-semibold text-sm rounded-xl active:scale-[0.98] transition-all shadow-sm"
                        style={{ background: 'var(--brand)', color: 'var(--btn-primary-text)' }}
                    >
