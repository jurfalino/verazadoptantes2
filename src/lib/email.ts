...
const OTP_COPY: Record<EmailLocale, { subject: string; greeting: string; expires: string; ignore: string }> = {
    es: {
        subject: 'Tu código de acceso a BuenAdoptante',
        greeting: 'Usá este código para iniciar sesión en BuenAdoptante:',
        expires: 'El código vence en 10 minutos.',
    ...
    pt: {
        ...
        expires: 'O código expira em 10 minutos.',
 ...
 */
export function buildOtpEmail(code: string, locale: EmailLocale): { subject: string; html: string; text: string } {
    const copy = OTP_COPY[locale] ?? OTP_COPY.es;
  ...
  <p style="margin:0 0 8px;font-size:14px;color:#57534e;">${copy.expires}</p>
