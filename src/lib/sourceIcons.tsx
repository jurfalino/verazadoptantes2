...
/**
 * Returns an SVG icon element matching the social network of a given URL.
 * Falls back to a generic link icon for unrecognized domains.
 */
export function getSourceIcon(url: string, className: string = 'w-4 h-4'): React.ReactNode {
    let hostname = '';
    try {
        hostname = new URL(url).hostname.toLowerCase();
    } catch {
        // If URL parsing fails, use generic icon
    }
    ...
    // Generic link icon for unknown sources
    return (
...
/**
 * Returns the display name of a social network from a URL.
 */
export function getSourceName(url: string): string {
    ...
    // Return clean hostname for unknown sources
