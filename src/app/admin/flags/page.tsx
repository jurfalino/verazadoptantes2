...
export default async function AdminFlagsPage() {
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            <h2 className="text-2xl font-bold text-stone-900">Flagged Content Review</h2>
                    ...
                    <div className="hidden md:block bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
                        <table className="w-full text-left">
                            <thead className="bg-stone-50 border-b border-stone-100">
                                <tr>
                                    <th className="p-4 font-bold text-stone-500 text-sm">Adopter</th>
                                    <th className="p-4 font-bold text-stone-500 text-sm">Reason</th>
                                    <th className="p-4 font-bold text-stone-500 text-sm">Details</th>
                                    <th className="p-4 font-bold text-stone-500 text-sm">Reporter</th>
                            ...
                            <tbody className="divide-y divide-stone-100">
                                {flags.map((flag: typeof flags[number]) => (
                                    <tr key={flag.id} className="hover:bg-stone-50/50">
                                        <td className="p-4">
                                            <div className="font-bold text-stone-900">{flag.adopterName || 'Unknown'}</div>
                                            <div className="text-xs text-stone-400 font-mono">{flag.adopterId}</div>
                                        </td>
                                        <td className="p-4">
                                            <span className={`inline-flex px-2 py-1 rounded-full text-xs font-bold uppercase ${flag.reason === 'dangerous' ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800'}`}>
                                                {flag.reason}
                                            </span>
                                        </td>
                                        ...
                                        <td className="p-4 text-right space-x-2">
                                            ...
                                            <form action={handleDismiss} className="inline-block">
                                                ...
                                                <button
                                                    type="submit"
                                                    className="px-3 py-1.5 text-xs font-bold text-white bg-teal-500 rounded-lg hover:bg-teal-600 shadow-sm"
                                                >
                                                    Dismiss
                                                </button>
                                            </form>
                            ...
                            <div key={flag.id} className="bg-white rounded-xl p-4 shadow-sm border border-stone-200">
                                <div className="flex items-start justify-between gap-2 mb-2">
                                    <div className="min-w-0">
                                        <div className="font-bold text-stone-900 truncate">{flag.adopterName || 'Unknown'}</div>
                                        <div className="text-xs text-stone-400 font-mono truncate">{flag.adopterId}</div>
                                    </div>
                                    <span className={`inline-flex px-2 py-1 rounded-full text-xs font-bold uppercase flex-shrink-0 ${flag.reason === 'dangerous' ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800'}`}>
                                        {flag.reason}
                                    </span>
                                </div>
                                {flag.details && (
                                ...
                                <div className="flex gap-2">
                                    <a
                                        href={`/adopter/${flag.adopterId}`}
                                        target="_blank"
                                        className="flex-1 text-center px-3 py-2 text-xs font-bold text-stone-600 bg-stone-100 rounded-lg hover:bg-stone-200"
                                    >
                                        View
                                    </a>
                                    <form action={handleDismiss} className="flex-1">
                                        ...
                                        <button
                                            type="submit"
                                            className="w-full px-3 py-2 text-xs font-bold text-white bg-teal-500 rounded-lg hover:bg-teal-600 shadow-sm"
                                        >
                                            Dismiss
                                        </button>
                                    </form>
