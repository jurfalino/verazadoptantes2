...
export async function addContactEntry(
    input: { adopterId: string; type: ContactEntry['type']; value: string; streetAndNumber?: string; locality?: string; platform?: SocialPlatform },
    ...
    try {
        ...
        const newEntry: ContactEntry = type === 'address' && (parsed.data.streetAndNumber || parsed.data.locality)
            ...
            : { id: crypto.randomUUID(), type, value, addedBy: actor, ...(type === 'social' && parsed.data.platform ? { platform: parsed.data.platform } : {}) };
